...
- **Měna (EUR)** — Shoptet.**cz** eshopy (gogrig.com, grig.cz, wormup.com, grigsupply.cz) ukládají EUR doklady **už převedené na CZK** v `cena_jedn_czk` + vyplněný `kurz_na_czk`. Shoptet.**sk** (grig.sk) posílá XML bez CZK kurzu (`homeCurrency` = EUR) → 🟢 OPRAVENO 2026-06-15: import (persist „Case B") u cizí měny bez kurzu dopočítá kurz přes `App\Service\CnbRateService` (denní kurzy ČNB dle DUZP, cache v `log/cnb/`), uloží `cena_jedn_mena` (EUR) + `cena_jedn_czk` (CZK) + `kurz_na_czk`. Migrace historických dat: `scripts/migrate_eur_to_czk.php` (dry-run / `--apply`). Diskriminátor řádků: `mena_puvodni <> 'CZK' AND (kurz_na_czk IS NULL OR kurz_na_czk = 0)`.
