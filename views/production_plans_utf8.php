...
<form method="get" action="/production/plans" class="product-filter-form">
  ...
  <label>
    <span>Znaďż˝,ďż˝?zďż˝aÂ¤ka</span>
    <select name="znacka_id">
      <option value="">VA"ďż˝.A<ďż˝??echny</option>
  ...
  <label>
    ...
    <span>Vyhledat</span>
    <input type="text" name="q" value="<?= htmlspecialchars($filterSearch, ENT_QUOTES, 'UTF-8') ?>" placeholder="SKU, nA"ďż˝?sA<ďż˝??zev, ALT SKU, EAN" />
  </label>
  <div class="search-actions">
    <?php if ($hasSearchActive): ?>
      <div class="search-result-pill">
        Zobrazeno <?= $resultCount ?>
        <a href="/production/plans" class="search-reset" title="ZruA"ďż˝.A<ďż˝??it filtr">&times;</a>
      </div>
    <?php endif; ?>
...
<?php if (!$hasSearchActive): ?>
  <div class="notice-empty">Zadejte parametry vyhledA"ďż˝?sA<ďż˝??vA"ďż˝?sA<ďż˝??nA"ďż˝?sA,Â­ a potvrďż˝,ďż˝?zďż˝aďż˝.te tlaďż˝,ďż˝?zďż˝aÂ¤A"ďż˝?sA,Â­tkem ďż˝,ďż˝'A?ďż˝?sÂ¬ďż˝aďż˝lVyhledatďż˝,ďż˝'A?ďż˝?sÂ¬ďż˝aďż˝?s. Seznam produktA"ďż˝.ďż˝aÂ» a nA"ďż˝?sA<ďż˝??vrh vA"ďż˝?sA<LAroby se zobrazA"ďż˝?sA,Â­ aA"ďż˝.A"ďż˝l po vyhledA"ďż˝?sA<ďż˝??nA"ďż˝?sA,Â­.</div>
<?php elseif (empty($items)): ?>
  <div class="notice-empty">Pro zadanA"ďż˝?sA,Â© podmA"ďż˝?sA,Â­nky nejsou dostupnA"ďż˝?sA<ďż˝?? A"ďż˝.A"ďż˝lA"ďż˝?sA<ďż˝??dnA"ďż˝?sA<ďż˝?? data.</div>
<?php else: ?>
  ...
  <table class="production-table">
    <thead>
      <tr>
        ...
        <th>Dostupnďż˝,Â©</th>
        <th>Rezervace</th>
        <th>Cďż˝,Â­lovďż˝,ďż˝t stav</th>
        <th>CA"ďż˝?sA,Â­lovA"ďż˝?sA<LA stav</th>
        <th>Dovyrobit</th>
        <th>Priorita</th>
        <th>Min. dA"ďż˝?sA<ďż˝??vka</th>
        <th>Krok vA"ďż˝?sA<LAroby</th>
        <th>VA"ďż˝?sA<LArobnA"ďż˝?sA,Â­ doba (dny)</th>
        <th>Akce</th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr class="<?= implode(' ', $rowClasses) ?>" data-sku="<?= htmlspecialchars($sku, ENT_QUOTES, 'UTF-8') ?>">
        <td class="sku-cell" data-sku="<?= htmlspecialchars($sku, ENT_QUOTES, 'UTF-8') ?>">
          <span class="sku-toggle">ďż˝,ďż˝'A?ďż˝,ďż˝ďż˝?sA,Â¸</span>
        ...
        <td>
          <form method="post" action="/production/produce" class="production-form" data-sku="<?= htmlspecialchars($sku, ENT_QUOTES, 'UTF-8') ?>">
            ...
            <input type="number" step="any" name="mnozstvi" placeholder="mnoA"ďż˝.A"ďż˝lstvA"ďż˝?sA,Â­" required />
            <button type="submit">Zapsat mnoA"ďż˝.A"ďż˝lstvA"ďż˝?sA,Â­</button>
          </form>
        </td>
      </tr>
      <?php endforeach; ?>
...
<div class="production-log-controls">
  <form method="post" action="/production/recent-limit">
    <label>PoA"Lďż˝et zobrazenďż˝,ďż˝tch zďż˝,ďż˝?znamďż˝aLďż˝:
  ...
  <table class="production-log-table">
    <thead>
      <tr>
        <th>Datum</th>
        <th>SKU</th>
        <th>Nďż˝ZLLďż˝tďż˝ZLLďż˝tzev</th>
        <th>Mnoďż˝ZLLďż˝tďż˝ZLLďż˝tstvďż˝ZLLďż˝tďż˝ZLLďż˝t</th>
      </tr>
    </thead>
    <tbody>
...
<?php else: ?>
  <p class="muted">Zatďż˝,Â­m nejsou zapsanďż˝,Â© ďż˝aďż˝lďż˝,ďż˝?dnďż˝,Â© vďż˝,ďż˝troby.</p>
<?php endif; ?>

<div class="production-modal-overlay" id="production-modal">
  <div class="production-modal">
    <h3>Nedostatek komponent</h3>
    <p>Odeďż˝,ďż˝?zďż˝aÂ¤et komponent by nďż˝,ďż˝?zA?ďż˝,ďż˝LzkterA"ďż˝?sA,Â© poloA"ďż˝.A"ďż˝lky poslal do zA"ďż˝?sA<ďż˝??pornA"ďż˝?sA,Â©ho stavu. Vyberte, jak postupovat:</p>
...
(function(){
  ...
  forms.forEach((form) => {
    form.addEventListener('submit', (event) => {
      ...
      checkDeficits(sku, qty)
        ...
        .catch((err) => alert('Nelze ovďż˝,ďż˝?zA?ďż˝,ďż˝LzA"ďż˝.A?ďż˝?zďż˝'it komponenty: ' + (err.message || err)));
    });
  });
  ...
  function toggleTreeRow(cell) {
    ...
    if (toggle) toggle.textContent = 'ďż˝,ďż˝'A?ďż˝,ďż˝ďż˝?sA"ďż˝l';
    row.classList.add('bom-open');
    const detailRow = document.createElement('tr');
    detailRow.className = 'production-tree-row';
    const detailCell = document.createElement('td');
    detailCell.colSpan = row.children.length;
    detailCell.textContent = 'Naďż˝,ďż˝?zďż˝aÂ¤A"ďż˝?sA,Â­tA"ďż˝?sA<ďż˝??m strom vazebďż˝,ďż˝'A?ďż˝?sÂ¬A,Â¦';
  ...
  function closeTreeRow() {
    ...
    if (toggle) toggle.textContent = 'ďż˝,ďż˝'A?ďż˝,ďż˝ďż˝?sA,Â¸';
  ...
  async function loadBomTree(sku, container) {
    if (!sku) {
      container.textContent = 'ChybA"ďż˝?sA,Â­ SKU.';
      return;
    }
    try {
      ...
      if (!data.ok) throw new Error(data.error || 'NepodaA"ďż˝.A?ďż˝?zďż˝'ilo se naďż˝,ďż˝?zďż˝aÂ¤A"ďż˝?sA,Â­st strom.');
      container.innerHTML = ';
  ...
  function buildBomTable(tree) {
    if (!tree || !Array.isArray(tree.children) || tree.children.length === 0) {
      ...
      wrap.textContent = 'Produkt nemA"ďż˝?sA<ďż˝?? navA"ďż˝?sA<ďż˝??zanA"ďż˝?sA,Â© potomky.';
      return wrap;
    }
    const table = document.createElement('table');
    table.className = 'bom-tree-table';
    table.innerHTML = '<thead><tr><th>Strom vazeb</th><th>Koeficient</th><th>MJ</th><th>Druh vazby</th><th>Typ poloA"ďż˝.A"ďż˝lky</th><th>DostupnA"ďż˝?sA,Â©</th><th>CA"ďż˝?sA,Â­lovA"ďż˝?sA<LA stav</th><th>ChybA"ďż˝?sA,Â­</th></tr></thead>';
    ...
    flattenTree(tree).forEach((row) => {
      ...
      label.textContent = `${row.node.sku} ďż˝,ďż˝'A?ďż˝?sÂ¬A?ďż˝,ďż˝L> ${row.node.nazev || '}`.trim();
  ...
  function buildPrefix(guides) {
    ...
    guides.forEach((guide, idx) => {
      ...
      if (idx === guides.length - 1) {
        prefix += isLast ? 'ďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝,ďż˝LAďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝?sÂ¬ďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝?sÂ¬ ' : 'ďż˝,ďż˝'A?ďż˝,ďż˝LAďż˝aďż˝?sďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝?sÂ¬ďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝?sÂ¬ ';
      } else {
        prefix += isLast ? '    ' : 'ďż˝,ďż˝'A?ďż˝,ďż˝LAA?ďż˝,ďż˝L?   ';
      }
    });
    return prefix;
  }
  ...
  function createCell(value) {
    ...
    td.textContent = value ?? 'ďż˝,ďż˝'A?ďż˝?sÂ¬A?ďż˝,ďż˝L>';
    return td;
  }

  function formatInteger(value) {
    if (value === null || value === undefined || isNaN(value)) {
      return 'ďż˝,ďż˝'A?ďż˝?sÂ¬A?ďż˝,ďż˝L>';
    }
